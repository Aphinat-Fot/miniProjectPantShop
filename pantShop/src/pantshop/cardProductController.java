...
public class cardProductController implements Initializable {
    ...
    public void setData(productData prodData) {
        ...
        color = prodData.getColor();
        prodID = prodData.getProductId();
        prod_name.setText(prodData.getProductName());
        prod_price.setText("$" + String.valueOf(prodData.getPrice()));
        String path = "File:" + prodData.getImage();
        image = new Image(path, 199, 97, false, true);
        prod_imageView.setImage(image);
        pr = prodData.getPrice();

    }

    public void setData1(productData prodData) {
        this.prodData = prodData;
        prod_image = prodData.getImage();
        prod_date = String.valueOf(prodData.getDate());
        color = prodData.getColor();
...
//                        + prod_name.getText() + "', color = '"
//                        + color + "', stock = 0, price = " + pr
...
//                            + "(customer_id, prod_id, prod_name, color, quantity, price, date, image) "
//                            + "VALUES(?,?,?,?,?,?,?,?)";
...
//    }
    ...
    public void addBtn1() {
        MenumainController test = new MenumainController();
        test.customerID1();
        qty = prod_spinner.getValue();
        String check = "";
        String checkAvailable = "SELECT status FROM product WHERE prod_id = '"
        ...
        try {
            ...
            if (!check.equals("มี") || qty == 0) {
                ...
                if (checkStck < qty) {
                    ...
                    String insertData = "INSERT INTO customer "
                            + "(customer_id, prod_id, prod_name, color, quantity, price, date, image,em_username) "
                            + "VALUES(?,?,?,?,?,?,?,?,?)";
                    prepare = connect.prepareStatement(insertData);
                    prepare.setString(1, String.valueOf(data.cID));
                    prepare.setString(2, prodID);
